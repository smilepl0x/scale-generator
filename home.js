let app = {
    ...
    scales: ["Ionian", "Dorian", "Phrygian", "Lydian", "Mixolydian", "Aeolian", "Locrian", "Harmonic minor","Locrian #6", "Ionian #5", "Dorian #4", "Phrygian #3", "Lydian #2", "Melodic minor", "Dorian b2", "Lydian augmented", "Lydian dominant", "Mixolydian b6", "Locrian ♮2", "Altered", "WH (fully diminished)", "HW (dominant diminished)", "Whole tone"]
}

// Create triad buttons
let triadDiv = document.getElementById("triads");
for (let triad of app.triads) {
    triadDiv.innerHTML += `<div class="btn btn-secondary triad-btn">${triad}</div>`;
}
...
for (let seventh of app.sevenths) {
    seventhsDiv.innerHTML += `<div class="btn btn-secondary seventh-btn">${seventh}</div>`;
}
...
for (let extension of app.extensions) {
    extensionsDiv.innerHTML += `<div class="btn btn-secondary extension-btn">${extension}</div>`;
}

// Create scales
let scalesDiv = document.getElementById("scales");
for (let scale of app.scales) {
    scalesDiv.innerHTML += `<div class="btn btn-secondary">${scale}</div>`;
}
